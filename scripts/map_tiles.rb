...
election_ids.each do |election_id|
  ...
  WHEN pp1.name = 'PP' THEN 
    ...
    END 
  WHEN pp1.name IN ('CIU', 'AP', 'IU', 'INDEP', 'CDS', 'PAR', 'EAJ-PNV', 'PA', 'BNG', 'PDP', 'ERC-AM', 'ESQUERRA-AM', 'ERC', 'EA', 'HB', 'PRC', 'PR', 'UV', 'EAJ-PNV/EA', 'EH', 'EAJ-ONV', 'PNV', 'EH-A', 'BILDU', 'PSC-PM', 'PSDEG-PSOE', 'PP-EU', 'IU-V-SIEX', 'IULV-CA', 'PSN-PSOE', 'PSE-EE (PSOE)', 'IUCYL', 'IU-LOS VERDES', 'PSOE DE ANDALUCIA') THEN
    pp1.name
  ELSE 'unknown' 
  END as color  
  FROM ine_poly AS g 
